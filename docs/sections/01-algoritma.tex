...
\subsubsection{Perbedaan Weighted A* dengan A* Biasa}
...
\subsubsection{Pengaruh Bobot terhadap Pemilihan State}

Nilai bobot $w$ menentukan seberapa besar pengaruh heuristik terhadap prioritas state. 
Semakin besar nilai $w$, semakin besar pengaruh $h(n)$ dibandingkan $g(n)$. 
Akibatnya, Weighted A* cenderung memilih state yang terlihat lebih dekat ke target, walaupun cost aktualnya belum tentu paling kecil.

\begin{tcolorbox}[
	colback=purple!5,
	colframe=purple!60!black,
	title=\textbf{Ilustrasi Pengaruh Bobot},
	arc=2mm,
	boxrule=0.8pt
]
Misalkan terdapat dua state berikut:
\[
\begin{array}{c|c|c|c}
\text{State} & g(n) & h(n) & g(n)+2h(n) \\
\hline
A & 10 & 7 & 24 \\
B & 14 & 3 & 20 \\
\end{array}
...
Weighted A* akan memilih state B karena nilai evaluasinya lebih kecil, meskipun cost aktual $g(n)$ state B lebih besar daripada state A.
\end{tcolorbox}

\subsubsection{Representasi State pada Weighted A*}

Representasi state pada Weighted A* sama dengan A*, karena keduanya membutuhkan informasi cost aktual dan heuristik. 
Setiap state menyimpan posisi aktor, progres angka yang sudah dikumpulkan, path gerakan, serta nilai cost yang digunakan dalam proses pencarian.

\begin{table}[H]
	\centering
	\caption{Representasi state pada Weighted A*}
	\begin{tabular}{|p{4cm}|p{10cm}|}
		\hline
		\rowcolor{purple!15}
		\textbf{Komponen State} & \textbf{Keterangan} \\
		\hline
		Posisi aktor & Menyimpan koordinat baris dan kolom aktor setelah suatu gerakan sliding. \\
		\hline
		\textit{Collected mask} & Menyimpan angka yang sudah dikumpulkan dalam bentuk bitmask. \\
		\hline
		Angka terakhir & Menyimpan angka terakhir yang berhasil dikumpulkan secara valid. \\
		\hline
		Path gerakan & Menyimpan urutan gerakan dari state awal sampai state saat ini. \\
		\hline
		\texttt{gCost} & Menyimpan total cost aktual dari start sampai state saat ini. \\
		\hline
		\texttt{hCost} & Menyimpan nilai estimasi dari state saat ini menuju target berikutnya atau goal. \\
		\hline
	\end{tabular}
\end{table}
...
\subsubsection{Alur Kerja Weighted A*}
...
\subsubsection{Skema Ekspansi Weighted A*}

\begin{tcolorbox}[
	colback=gray!5,
	colframe=black!60,
	title=\textbf{Skema Ekspansi Weighted A*},
	arc=2mm,
	boxrule=0.8pt
]
\small
...
\centering
\textbf{1. Ambil State}\\
Ambil state dengan nilai $g(n)+2h(n)$ terkecil dari priority queue.
}} \\[0.25cm]
$\downarrow$ \\[-0.05cm]
\fbox{\parbox{0.82\textwidth}{
\centering
\textbf{2. Cek Pruning dan Goal}\\
Abaikan jika sudah ada cost lebih baik. Jika posisi aktor di \texttt{O} dan semua angka sudah dikumpulkan, solusi ditemukan.
}} \\[0.25cm]
$\downarrow$ \\[-0.05cm]
\fbox{\parbox{0.82\textwidth}{
\centering
\textbf{3. Ekspansi Empat Arah}\\
Coba gerakan \texttt{U}, \texttt{D}, \texttt{L}, dan \texttt{R} menggunakan \texttt{MovementEngine::slide}.
}} \\[0.25cm]
$\downarrow$ \\[-0.05cm]
\fbox{\parbox{0.82\textwidth}{
\centering
\textbf{4. Bentuk State Baru}\\
Hitung \texttt{newGCost}, bentuk path baru, hitung ulang heuristik, lalu masukkan ke priority queue jika lebih baik.
}}
\end{tabular}
\end{center}
\end{tcolorbox}

\subsubsection{Penanganan Gerakan Tidak Valid dan Game Over}

Weighted A* tidak langsung menganggap semua arah sebagai successor yang valid. 
Setiap arah harus disimulasikan terlebih dahulu menggunakan \texttt{MovementEngine::slide}. 
Jika hasil gerakan tidak valid atau menyebabkan game over, state tersebut tidak dimasukkan ke dalam priority queue.

\begin{table}[H]
	\centering
	\caption{Penanganan aturan puzzle pada Weighted A*}
	\begin{tabular}{|p{4cm}|p{10cm}|}
		\hline
		\rowcolor{purple!15}
		\textbf{Kondisi} & \textbf{Penanganan} \\
		\hline
		Gerakan tidak valid & State hasil tidak diproses dan algoritma lanjut mencoba arah lain. \\
		\hline
		Game over & Terjadi apabila aktor melewati lava, keluar papan, atau melanggar urutan angka; state langsung diabaikan. \\
		\hline
		Rintangan \texttt{X} & Aktor berhenti tepat sebelum rintangan. \\
		\hline
		Angka berurutan & Collected mask dan angka terakhir diperbarui jika angka yang dilewati sesuai urutan. \\
		\hline
		State dengan cost lebih buruk & Diabaikan jika sudah ada state dengan posisi dan collected mask sama serta cost lebih kecil atau sama. \\
		\hline
	\end{tabular}
\end{table}

\subsubsection{Output Weighted A*}

Weighted A* menghasilkan output dengan format yang sama seperti solver lainnya. 
Apabila solusi ditemukan, program mengembalikan path solusi, total cost, waktu eksekusi, dan jumlah iterasi. 
Apabila solusi tidak ditemukan, program mengembalikan status gagal dan pesan bahwa solusi tidak ditemukan.

\begin{table}[H]
	\centering
	\caption{Output Weighted A*}
	\begin{tabular}{|p{4cm}|p{10cm}|}
		\hline
		\rowcolor{purple!15}
		\textbf{Kondisi} & \textbf{Output} \\
		\hline
		Solusi ditemukan & Status solusi ditemukan, path solusi, total cost, waktu eksekusi dalam milidetik, dan jumlah iterasi. \\
		\hline
		Solusi tidak ditemukan & Status solusi tidak ditemukan, pesan \texttt{Solusi tidak ditemukan.}, waktu eksekusi, dan jumlah iterasi. \\
		\hline
	\end{tabular}
\end{table}

\subsubsection{Kelebihan dan Kekurangan Weighted A*}
